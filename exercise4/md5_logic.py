...
def md5_hash_with_steps(message: bytes) -> dict:
    """
    MD5 hash function that returns both the final hash and intermediate steps for educational purposes.
    """
    steps = {
        'initial_values': {},
        'blocks': [],
    ...
    for block_idx, block in enumerate(chunks):
        ...
        for k in range(16):
            chunk_4bytes = block[k*4 : (k+1)*4]
            word_val = int.from_bytes(chunk_4bytes, byteorder='little')
            M.append(word_val)
            block_info['message_words'].append(f"M[{k}] = {word_val:08x}")

        # Initialize hash values for this block
        A, B, C, D = a0, b0, c0, d0

        block_info['rounds'].append({
            'round': 0,
            'A': f"{A:08x}",
            'B': f"{B:08x}",
            'C': f"{C:08x}",
            'D': f"{D:08x}",
            'description': 'Initial values for this block'
        })
        ...
        for j in range(64):
            ...
            D = C
            C = old_B
            B = (old_B + left_rotate(temp, shifts[j])) & 0xFFFFFFFF
            ...
            round_info = {
                ...
                'K_value': f"{K[j]:08x}",
                'shift': shifts[j],
                'temp_calc': f"({f_name} + A + K[{j}] + M[{g_idx}]) = ({f_val:08x} + {A:08x} + {K[j]:08x} + {M[g_idx]:08x})",
                'temp': f"{temp:08x}",
                'rotated': f"left_rotate({temp:08x}, {shifts[j]}) = {left_rotate(temp, shifts[j]):08x}",
                'A': f"{A:08x}",
                'B': f"{B:08x}",
                'C': f"{C:08x}",
                'D': f"{D:08x}"
            }
            block_info['rounds'].append(round_info)

        # Add this block's hash to the cumulative sum
        old_a0, old_b0, old_c0, old_d0 = a0, b0, c0, d0
        a0 = (a0 + A) & 0xFFFFFFFF
        b0 = (b0 + B) & 0xFFFFFFFF
        c0 = (c0 + C) & 0xFFFFFFFF
        d0 = (d0 + D) & 0xFFFFFFFF

        block_info['cumulative_sum'] = {
            'before': f"A={old_a0:08x}, B={old_b0:08x}, C={old_c0:08x}, D={old_d0:08x}",
            'added': f"A+={A:08x}, B+={B:08x}, C+={C:08x}, D+={D:08x}",
            'after': f"A={a0:08x}, B={b0:08x}, C={c0:08x}, D={d0:08x}"
        }

        steps['blocks'].append(block_info)
    ...
    steps['final_hash'] = final_hash.hex()
